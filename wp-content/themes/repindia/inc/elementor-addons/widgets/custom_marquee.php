...
class Custom_Marquee extends Widget_Base
{
    ...
    protected function render()
    {
        ...
        if (!$js_added) {
            ...
            echo '(function() {';
            echo 'var marquees = [], rafId = null, lastTime = 0, mobileQuery = window.matchMedia("(max-width: 768px)"), mobileSpeedMultiplier = 2;';
            echo 'function isMobileView() { return mobileQuery.matches; }';
            echo 'function getEffectiveSpeed(baseSpeed) {';
            echo 'return isMobileView() ? baseSpeed * mobileSpeedMultiplier : baseSpeed;';
            echo '}';
            echo 'function updateMarqueeMetrics(m) {';
            echo 'var loopWidth = m.track.offsetWidth / 2;';
            echo 'if (loopWidth <= 0) return false;';
            echo 'm.loopWidth = loopWidth;';
            echo 'm.velocity = loopWidth / getEffectiveSpeed(m.baseSpeed);';
            echo 'if (m.pos >= m.loopWidth) m.pos = m.pos % m.loopWidth;';
            echo 'return true;';
            echo '}';
            ...
            echo 'var baseSpeed = parseFloat(wrapper.dataset.speed) || 40;';
            echo 'var direction = wrapper.dataset.direction || "left";';
            echo 'var pauseOnHover = wrapper.dataset.hover === "yes";';
            echo 'var existing = marquees.find(function(m) { return m.wrapper === wrapper; });';
            echo 'if (existing) { updateMarqueeMetrics(existing); return; }';
            echo 'wrapper.dataset.initialized = "1";';
            echo 'var m = { wrapper: wrapper, track: track, loopWidth: 0, baseSpeed: baseSpeed, velocity: 0, direction: direction, pauseOnHover: pauseOnHover, pos: 0, hovered: false };';
            echo 'if (!updateMarqueeMetrics(m)) return;';
            ...
            echo '}';
            echo 'function handleMarqueeResize() { marquees.forEach(function(m) { updateMarqueeMetrics(m); }); }';
            echo 'window.addEventListener("resize", handleMarqueeResize);';
            echo 'if (typeof mobileQuery.addEventListener === "function") mobileQuery.addEventListener("change", handleMarqueeResize);';
            echo 'else if (typeof mobileQuery.addListener === "function") mobileQuery.addListener(handleMarqueeResize);';
            echo 'window.addEventListener("load", initCustomMarquees);';
            echo 'function run() {';
